...
1. **Run the Sorting Program**
    ...
    Arguments
    1. -f or -F followed by file_name (the file name and path) with no spaces
    2. -t or -T followed by v (volume), h (height) or a (base area) with no spaces
    3. -s or -S followed by b (bubble), s (selection), i (insertion), m (merge), q (quick) or z
